...
$q = "SELECT * FROM `sample` WHERE Email='$email' && Password='$password'";
...
if ($rq = mysqli_query($con, $q)) {
    ...
    if (mysqli_num_rows($rq) == 1) {
        ...
        $q = "SELECT * FROM `sample` WHERE Password='$password'";
        if ($rq = mysqli_query($con, $q)) {
            if (mysqli_num_rows($rq) == 1) {
                echo "<script>alert($password+' is already taken by another person')</script>";
            } 
            // else {
